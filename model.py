...
class Static_Model(nn.Module):
    ...
    def forward(self, images,depth_maps,head,face, left_eye, right_eye):
        ...
        depth_feat = self.avgpool(depth_feat).view(-1, 1024)
        ...
        attn_weights = self.attn2(torch.cat((head_reduced, base_face, depth_feat), 1))
...
class Complete_Model(nn.Module):
    ...
    def forward(self, images, depth_images, head, face, left_eye, right_eye, hidden_scene: tuple = None, batch_sizes: list = None):
        face = self.conv1_face(face)
        face = self.bn1_face(face)
        face = self.relu(face)
        face = self.maxpool(face)
